...
const Vehicles = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      setVehicles([
        { vehicle_id: "CAB-1023", make: "Toyota", model: "Camry", body_type: "Sedan", license_status: "ACTIVE" },
        { vehicle_id: "SDF-4492", make: "Honda", model: "Civic", body_type: "Sedan", license_status: "SUSPENDED" },
        { vehicle_id: "ZXC-9901", make: "Ford", model: "F-150", body_type: "Truck", license_status: "ACTIVE" }
      ]);
      setLoading(false);
    }, 600);
  }, []);

  const filtered = vehicles.filter(v => v.vehicle_id.toLowerCase().includes(search.toLowerCase()));
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/60 backdrop-blur-lg p-5 rounded-xl shadow-md border border-white/60 gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-transparent text-sm transition-all shadow-sm"
          />
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {filtered.map(vehicle => (
                  <tr key={vehicle.vehicle_id} className="hover:bg-slate-50 even:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-800">{vehicle.vehicle_id}</td>
                    <td className="px-6 py-4 text-slate-600">{vehicle.make} {vehicle.model}</td>
                    <td className="px-6 py-4 text-slate-500 text-sm">{vehicle.body_type}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${
                        vehicle.license_status === "ACTIVE" 
